...
if (isset($_SESSION['name']) == true && isset($_SESSION['quyen']) == true && isset($_SESSION['idUser'])) {
	...
	<script type="text/javascript">
		document.addEventListener('DOMContentLoaded', function()
		{
			...
			var arr = [<?php foreach ($all as $value){echo '"'. $value['maNhapKho'] .'",'; } ?>];

			document.querySelector('#maNhapKho').oninput = function()
			{

				var checkk = arr.filter(arr => arr == document.querySelector('#maNhapKho').value);
				var s_checkk = String(checkk);

				if (document.querySelector('#maNhapKho').value != "")
				{
					if (s_checkk == document.querySelector('#maNhapKho').value) 
					{
			...
			{
				var maNhapKho = document.querySelector('#maNhapKho').value;
		...
		<div class="col-md-7 boder-right">
			<div class="container">
				...
				<form action="" method="POST">
					<table class="table" border="1">
						...
						<tbody>
							...
							if (isset($_SESSION["cart-nhap"])) {
								...
								{
									$tong = $value["gianhap"] * $value["sl_muonnhap"];
									$tongtien += $tong;
									...
									<tr>
										...
										<td>
											<input type="number" min="1" name="sl_muonnhap<?php echo $value["idNVL"]; ?>" value="<?php echo $value["sl_muonnhap"]; ?>">
										</td>
					...
					<div>
						<p>Tổng giá tiền nhập kho là : <?php echo number_format($tongtien); ?> (VNĐ)</p>
					</div>
					<div>
						<button type="submit" class="bg-danger" name="update">Cập nhật danh sách</button>
					</div>
					<div class="form-group">
						<label for="maNhapKho">Mã nhập kho</label>
						<input type="text" class="form-control" id="maNhapKho" placeholder="Nhập mã nhập kho" name="maNhapKho">
					...
					<div class="form-group">
						<input  class="bg-danger" onclick="return confirm('Bạn đã cập nhật danh sách trước khi gửi chưa?');" type="submit" name="submit" id="submit" value="Nhập kho">
					</div>
				</form>
			</div>
		</div>
