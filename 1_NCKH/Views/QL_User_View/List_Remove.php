...
if (isset($_SESSION['name']) == true && isset($_SESSION['quyen']) == true && isset($_SESSION['idUser'])) {
	...
	<head>
		...
		<title>TOYOTA (Tra cứu thông tin account không hoạt động)</title>
		<meta charset="utf-8">
	...
	<div class="row mt-5 mb-5" >
		...
		<div class="col-md-1 mx-5 boder-right">
			<div id="mySidenav" class="sidenav">
				...
				<ol>
					<ul>
						<li><a href="index.php?controller=QL_Kho&action=List&key=">Quản lí nhập/xuất kho</a></li>
					</ul>
				</ol>
				<hr>
		...
		<div class="col-md-7 boder-right">
			<center>
				<h3>Bảng thông tin account không còn hoạt động</h3>
				<table border="1">
					<thead>
						<tr>
							<th>STT</th>
							...
							foreach ($data_Search as $values)
							{
								...
								<tr>
									...
									<td><?php echo $values['tenNV']; ?></td>
										<td><a onclick="return confirm('Bạn có muốn khôi phục không')" href="index.php?controller=QL_User&action=Restore&idUser=<?php echo $values['idUser']; ?>">Khôi phục</a></td>
										<td>
											<a onclick="return confirm('Bạn có muốn xóa hoàn toàn không')" href="index.php?controller=QL_User&action=Delete&idUser=<?php echo $values['idUser']; ?>">Xóa hoàn toàn</a>
										</td>
								</tr>
								<?php 
								$stt++;
