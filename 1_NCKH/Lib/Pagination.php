...
class Pagination extends DB
{
     ...
     * @return int
     */
     public function gettotalRecord_KH()
     {
        $conn = $this->connect();

        $result = $conn->query('SELECT count(idKhachHang) AS total FROM khachhang');
    ...
    /**
     * Lấy ra tổng số record được tìm theo start và limit
     *
     * @return string
     */
    public function getAllData_KH()
    {
    ...
    {
        $tranghientai = $this->getCurrentPage();
        $limit = $this->config['limit'];
        $data = (($tranghientai - 1) * $limit );
        return $data;
    }
}